...
final class GenerateReceiptsForMonth
{
    ...
    public function execute(string $month): GenerateReceiptsResult
    {
        ...
        try {
            ...
            // Bornes explicites de la période de location (du 1er au dernier jour du mois)
            $startDate = new DateTimeImmutable(sprintf('%04d-%02d-01', $m->year(), $m->month()));
            ...
            foreach ($rows as $row) {
                ...
                $vars = [
                    ...
                    'services_amount_eur' => $this->formatCentsToEur((int) ($row['services_amount'] ?? 0)),
                    'total_amount_eur' => $this->formatCentsToEur($rentCents + $chargesCents + (int) ($row['services_amount'] ?? 0)),
                ];
